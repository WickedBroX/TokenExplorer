...
const TOKEN_INFO_CACHE_KEY = 'bzr:lastTokenInfo';
const TOKEN_INFO_CACHE_TTL_MS = 12 * 60 * 60 * 1000; // 12 hours
...
export const useTokenData = (): UseTokenDataResult => {
  ...
  const infoRef = useRef<TokenInfo | null>(null);
      ...
      try {
        ...
        if (raw) {
          ...
          if (Array.isArray(parsed.data)) {
            const cachedTransfers = parsed.data as Transfer[];
  ...
  useEffect(() => {
    ...
    try {
      const raw = window.localStorage.getItem(TOKEN_INFO_CACHE_KEY);
      if (!raw) {
        return;
      }

      const parsed = JSON.parse(raw) as { data?: TokenInfo; timestamp?: number };
      if (!parsed?.data || !parsed.data.tokenName) {
        return;
      }

      if (typeof parsed.timestamp === 'number' && (Date.now() - parsed.timestamp) > TOKEN_INFO_CACHE_TTL_MS) {
        window.localStorage.removeItem(TOKEN_INFO_CACHE_KEY);
        return;
      }

      setInfo(parsed.data);
      infoRef.current = parsed.data;
      setInfoLoadingState(false);
    } catch (storageError) {
      console.warn('Failed to restore token info cache:', storageError);
    }
  }, [setInfoLoadingState]);
  ...
  const fetchData = useCallback(async function fetchDataInner({ mode = 'initial', force = false }: FetchOptions = {}) {
    ...
    if (isInitial) {
      ...
      if (!transfersRef.current.length) {
  ...
  const shouldFetchInfo = isInitial || isRefresh || force || !infoRef.current;
    ...
    const infoPromise = shouldFetchInfo
      ? (async () => {
          try {
            ...
            if (typeof window !== 'undefined') {
              try {
                window.localStorage.setItem(
                  ...
                  JSON.stringify({ data: infoData, timestamp: Date.now() })
                );
              } catch (cacheError) {
                console.warn('Failed to persist token info cache:', cacheError);
              }
            }
          } catch (err) {
            if (!mountedRef.current) return;
            ...
            const message = (err as Error).message || 'Failed to load token info';
  ...
  transfersRef.current = normalizedTransfers;
  ...
  }, [setInfoLoadingState, setTransfersLoadingState, setRefreshing]);

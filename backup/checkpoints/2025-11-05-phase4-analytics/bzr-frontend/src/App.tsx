...
export default function App() {
  ...
  const holderMetrics = useMemo(() => {
    ...
    // Pie chart data: Top 10 individually, 11-50 grouped, Rest grouped
    const pieChartData = [];
    const top10 = balances.slice(0, Math.min(10, balances.length));
    top10.forEach((balance, index) => {
      pieChartData.push({
        name: `#${index + 1} Holder`,
        value: balance,
    ...
    if (balances.length > 10) {
      ...
      if (top1150Sum > 0) {
        pieChartData.push({
          name: 'Top 11-50',
          value: top1150Sum,
          percentage: (top1150Sum / totalSupply) * 100
        });
      }
    }
    
    if (balances.length > 50) {
      const restSum = balances.slice(50).reduce((sum, b) => sum + b, 0);
      if (restSum > 0) {
        pieChartData.push({
          name: 'Others',
          value: restSum,
          percentage: (restSum / totalSupply) * 100
        });
      }
    }
                    ...
                    <div className="bg-gradient-to-br from-green-50 via-white to-blue-50 shadow-xl rounded-lg p-8 border border-gray-100">
                      <div className="flex items-center gap-6">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-500 mt-2">The Ultimate Multichain Transaction Explorer for BZR Token</p>
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="bg-white shadow-xl rounded-lg p-6 border border-gray-100">
                          ...
                          <div className="space-y-3">
                            <div className="p-3 rounded-lg bg-gradient-to-r from-blue-50 to-white border border-blue-100">
                              ...
                              <p className="text-base font-semibold text-gray-900 mt-1">{info.tokenName}</p>
                            </div>
                            <div className="p-3 rounded-lg bg-gradient-to-r from-purple-50 to-white border border-purple-100">
                              <span className="text-xs text-gray-500 uppercase tracking-wide">Symbol</span>
                              <p className="text-base font-semibold text-gray-900 mt-1">{info.tokenSymbol}</p>
                            </div>
                            <div className="p-3 rounded-lg bg-gradient-to-r from-green-50 to-white border border-green-100">
                              ...
                              <p className="text-base font-semibold text-gray-900 mt-1">{info.tokenDecimal}</p>
                            </div>
                    ...
                    <div className="p-4 sm:p-6 border-b border-gray-200">
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          >
                            {availableChains
                              .filter(chain => chain.id !== 25) // Exclude Cronos (unsupported)
                              .map(chain => (
                      ...
                      <div className="px-4 sm:px-6 pb-4">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          ...
                          <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            <h4 className="text-sm font-semibold text-gray-900 mb-4">Holder Distribution</h4>
                            <ResponsiveContainer width="100%" height={300}>
                              <PieChart>
                                <Pie
                                  data={holderMetrics.pieChartData}
                                  cx="50%"
                                  cy="50%"
                                  labelLine={false}
                                  ...
                                  dataKey="value"
                                >
                                  {holderMetrics.pieChartData.map((_entry, index) => {
                                    const colors = ['#3b82f6', '#8b5cf6', '#10b981', '#f59e0b', '#ef4444', '#06b6d4', '#ec4899', '#6366f1', '#14b8a6', '#f97316', '#64748b', '#84cc16'];
                                    return <Cell key={`cell-${index}`} fill={colors[index % colors.length]} />;
                                  })}
                                </Pie>
                                <Tooltip 
                                  formatter={(value: number) => `${value.toLocaleString(undefined, { maximumFractionDigits: 0 })} BZR`}
                                />
                                <Legend 

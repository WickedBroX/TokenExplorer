...
const { respondUpstreamFailure, markApiKeyAsFailed, shouldBackoffApiKey } = require('../utils/apiUtils');
...
const fetchStatsForChainOnce = async (chain) => {
  ...
  try {
    ...
    if (apiKey && shouldBackoffApiKey(error)) {
      markApiKeyAsFailed(apiKey, { reason: `holdercount_${chain.id}_rate_limit` });
    }

...
const buildProviderRequest = (chain, params = {}, options = {}) => {
  ...
  if (includeApiKey) {
    if (provider.key === 'etherscan') {
      selectedApiKey = getNextApiKey();
      if (!selectedApiKey) {
        const error = new Error('Missing Etherscan API key');
        error.code = 'MISSING_ETHERSCAN_KEY';
        throw error;
      }
      nextParams.apikey = selectedApiKey;
    } else {
  ...
  return { provider, params: nextParams, apiKey: selectedApiKey };
};

        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4">
            ...
            <DetailRow 
              ...
              link={getExplorerUrl(transaction.chainName, transaction.hash).replace(`tx/${transaction.hash}`, `address/${transaction.from}`)}
              copyable 
            />
            <DetailRow 
              label="To" 
              value={transaction.to}
              link={getExplorerUrl(transaction.chainName, transaction.hash).replace(`tx/${transaction.hash}`, `address/${transaction.to}`)}
              copyable 
            />
...
export default function App() {
  ...
  useEffect(() => {
    if (activeTab === 'analytics') {
      // Fetch 500 transfers for better historical analytics data
      if (transfersQuery.pageSize < 500) {

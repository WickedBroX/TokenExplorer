...
const runMigrations = async (clientPool) => {
  ...
  try {
    ...
    // See migrations/002-create-transfer-tables.sql
    // The application assumes these tables exist.
...
const deriveLogIndex = (transfer) => {
  const rawLogIndex = transfer.logIndex ?? transfer.log_index ?? transfer.transactionLogIndex;
  const explicit = rawLogIndex === undefined ? null : Number.parseInt(rawLogIndex, 10);
  if (Number.isFinite(explicit) && explicit >= 0) {
    return explicit;
  }

  const components = [
    transfer.transactionIndex || transfer.transaction_index || '',
    transfer.nonce || '',
    transfer.from || transfer.fromAddress || transfer.from_address || '',
    transfer.to || transfer.toAddress || transfer.to_address || '',
    transfer.value || '',
  ].map((value) => (typeof value === 'string' ? value.toLowerCase() : String(value)));

  const digest = crypto.createHash('md5').update(components.join('|')).digest('hex');
  const synthetic = Number.parseInt(digest.slice(0, 7), 16);
...
const normalizeTransferRecord = (chainId, transfer) => {
  ...
  const logIndex = deriveLogIndex(transfer);
...
const updateUpstreamTotals = async (chainId, upstreamTotal, upstreamLastBlock) => {
  if (!storeEnabled || !storeReady) {
    return null;
  }
  ...
  const result = await withClient((client) => client.query(
    `
      ...
      ON CONFLICT (chain_id) DO UPDATE SET
        upstream_total_transfers = EXCLUDED.upstream_total_transfers,
        upstream_last_block = EXCLUDED.upstream_last_block,
        upstream_updated_at = NOW(),
        updated_at = NOW()
      RETURNING chain_id, upstream_total_transfers, upstream_last_block, upstream_updated_at;
    `,
    [chainId, upstreamTotal, upstreamLastBlock]
  ));
  ...
  return {
    chainId: row.chain_id,
    upstreamTotalTransfers: row.upstream_total_transfers ? Number.parseInt(row.upstream_total_transfers, 10) : null,
    upstreamLastBlock: row.upstream_last_block ? Number.parseInt(row.upstream_last_block, 10) : null,
    upstreamUpdatedAt: row.upstream_updated_at ? new Date(row.upstream_updated_at) : new Date(),
  };
};
  ...
  const result = await withClient((client) => client.query(
    `
      ...
      SELECT
        ...
        totals.upstream_total_transfers,
        totals.upstream_last_block,
        totals.upstream_updated_at,
        status.ready,
  ...
  return result.rows.map((row) => {
    ...
    return {
      ...
      totals: {
        ...
        upstreamTotalTransfers: row.upstream_total_transfers ? Number.parseInt(row.upstream_total_transfers, 10) : null,
        upstreamLastBlock: row.upstream_last_block ? Number.parseInt(row.upstream_last_block, 10) : null,
        upstreamUpdatedAt: row.upstream_updated_at ? new Date(row.upstream_updated_at) : null,
        totalVolumeRaw: row.total_volume_raw ? String(row.total_volume_raw) : '0',
        firstTime: row.first_time ? new Date(row.first_time) : null,
        lastTime,
...
const buildFilterClause = ({ chainId, startBlock, endBlock, startTime, endTime, filterAddress, filterHash }) => {
  const conditions = [];
  const values = [];
  ...
  if (filterAddress) {
    values.push(filterAddress.toLowerCase());
    conditions.push(`(from_address = $${values.length} OR to_address = $${values.length})`);
  }

  if (filterHash) {
    values.push(filterHash.toLowerCase());
    conditions.push(`tx_hash = $${values.length}`);
  }
...
const countTransfers = async ({ chainId, startBlock, endBlock, filterAddress, filterHash }) => {
  if (!storeEnabled || !storeReady) {
    throw new Error('Persistent store unavailable');
  }

  const chainFilter = Array.isArray(chainId) ? chainId : [chainId];
  const { clause, values } = buildFilterClause({ chainId: chainFilter, startBlock, endBlock, filterAddress, filterHash });

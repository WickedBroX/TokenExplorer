...
const { getNextApiKey, isProOnlyResponse, markApiKeyAsFailed, shouldBackoffApiKey } = require('../utils/apiUtils');
...
const fetchTokenPriceFromEtherscan = async () => {
  ...
  try {
    ...
    if (shouldBackoffApiKey(error)) {
      markApiKeyAsFailed(apiKey, { reason: 'token_price_rate_limit' });
    }

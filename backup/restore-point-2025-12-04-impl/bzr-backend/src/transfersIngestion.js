...
const TOTALS_SYNC_INTERVAL_MS = Number(process.env.TRANSFERS_TOTALS_SYNC_INTERVAL_MS || 10 * 60 * 1000);
...
const startTransferIngestion = ({ chains, fetchPage, fetchTotal, logger = console }) => {
  ...
  const scheduleChain = (chain, state, delayOverride = null) => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (fetchTotal && (!state.lastTotalSync || Date.now() - state.lastTotalSync > TOTALS_SYNC_INTERVAL_MS)) {
          try {
            const totalResult = await fetchTotal({ chain });
            if (totalResult && typeof totalResult.total === 'number') {
              await updateUpstreamTotals(chain.id, totalResult.total, null);
              state.lastTotalSync = Date.now();
              if (logger?.info) {
                logger.info(`✓ Synced upstream totals for ${chain.name}: ${totalResult.total}`);
              } else {
                console.log(`✓ Synced upstream totals for ${chain.name}: ${totalResult.total}`);
              }
            }
          } catch (err) {
            console.error(`X Failed to sync upstream totals for ${chain.name}:`, err.message);
          }
        }

...
const RETRY_ATTEMPTS = Number(process.env.SEED_RETRY_ATTEMPTS || 5);
const RETRY_DELAY_MS = Number(process.env.SEED_RETRY_DELAY_MS || 2000);
const PAGE_DELAY_MS = Number(process.env.SEED_PAGE_DELAY_MS || 500);
const ENV_START_PAGE = Number(process.env.SEED_START_PAGE);
...
const showHelp = () => {
  ...
  console.log('  --start-page=<n>    Page to start from (descending), default 1');
...
const options = {
  ...
  startPage: Number.isFinite(ENV_START_PAGE) ? ENV_START_PAGE : parseNumberOption('start-page', 1),
...
const seedChain = async (chain) => {
  ...
  let page = 1;
  if (options.startPage && Number.isFinite(options.startPage)) {
    page = Math.max(1, options.startPage);
  }
  let insertedTotal = 0;
  ...
  console.log(`\n[${chain.name}] Starting seed: pages=${options.pages}, pageSize=${options.pageSize}, startPage=${page}`);
  ...
  while (page <= options.pages) {
    ...
    const fetchPageWithRetry = async () => {
      ...
      while (attempt <= RETRY_ATTEMPTS) {
        try {
          return await fetchTransfersPageFromChain({
            chain,
            page,
          ...
          console.warn(`[${chain.name}] Page ${page} attempt ${attempt} failed (${error.message || error}). Retrying in ${delay}ms...`);
          await sleep(delay);
          attempt += 1;
        }
      }
...
const run = async () => {
  ...
  await Promise.all(chains.map((chain) => limit(async () => {
    try {
      ...
      if (error.payload) {
        console.error(`[${chain.name}] Upstream payload:`, JSON.stringify(error.payload));
      }

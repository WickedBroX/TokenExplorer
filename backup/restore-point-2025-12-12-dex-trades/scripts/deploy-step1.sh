...
"${SSH_CMD[@]}" ${SERVER_USER}@${SERVER} "mkdir -p ${SERVER_PATH}/migrations ${SERVER_PATH}/scripts"
echo "✅ Directories ready"
echo ""

# Step 1: Copy migration file
echo "📦 Step 1/7: Uploading database migration..."
"${SCP_CMD[@]}" bzr-backend/migrations/001-create-token-info.sql ${SERVER_USER}@${SERVER}:${SERVER_PATH}/migrations/
echo "✅ Migration uploaded"
echo ""

# Step 2: Copy refresh script
echo "📦 Step 2/7: Uploading refresh script..."
"${SCP_CMD[@]}" bzr-backend/scripts/refresh-token-info.js ${SERVER_USER}@${SERVER}:${SERVER_PATH}/scripts/
echo "✅ Refresh script uploaded"
echo ""

# Step 3: Copy updated server.js
echo "📦 Step 3/7: Uploading updated server.js..."
"${SCP_CMD[@]}" bzr-backend/server.js ${SERVER_USER}@${SERVER}:${SERVER_PATH}/
echo "✅ Server.js uploaded"
echo ""

# Step 4: Run migration
echo "🗄️  Step 4/7: Running database migration..."
"${SSH_CMD[@]}" ${SERVER_USER}@${SERVER} "cd ${SERVER_PATH} && sudo -u postgres psql -d ${DB_NAME} -f migrations/001-create-token-info.sql"
echo "✅ Migration completed"
echo ""

# Step 5: Initial population (force refresh)
echo "🔄 Step 5/7: Running initial token info refresh..."
"${SSH_CMD[@]}" ${SERVER_USER}@${SERVER} "cd ${SERVER_PATH} && node scripts/refresh-token-info.js --force"
echo "✅ Initial refresh completed"
echo ""

# Step 6: Restart backend service
echo "🔄 Step 6/7: Restarting backend service..."
"${SSH_CMD[@]}" ${SERVER_USER}@${SERVER} "systemctl restart bzr-backend.service"
...
"${SSH_CMD[@]}" ${SERVER_USER}@${SERVER} "systemctl is-active bzr-backend.service" || echo "⚠️  Service check failed"
echo ""
...
"${SSH_CMD[@]}" ${SERVER_USER}@${SERVER} "sudo -u postgres psql -d ${DB_NAME} -c 'SELECT contract_address, token_name, token_symbol, updated_at FROM token_info;'" || echo "⚠️  Database check failed"
echo ""

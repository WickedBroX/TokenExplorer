          ...
          <div className="divide-y divide-gray-100">
            {loadingHolders ? (
              ...
              holderMetrics.map(({ holder, balance, percent, barWidth, valueUsd, rankLabel }) => {
                const address = normalizeAddress(holder.TokenHolderAddress);
                const validAddress = isValidAddress(address);
                const truncatedAddress = address ? truncateHash(address) : "—";
                const rowKey = `${address || "unknown"}-${holder.chainId ?? holder.chainName ?? chainName}`;
                return (
                  <div key={rowKey} className="p-3 bg-white">
                        ...
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          <div className="flex items-center gap-1 min-w-0">
                            {validAddress ? (
                              ...
                              <span className="text-gray-500 font-mono truncate text-sm min-w-0">
                                {truncatedAddress}
                              </span>
                            )}
                          </div>
                          <button
                            onClick={() =>
                              address && handleCopyAddress(address)
                            }
                    ...
                    <td className="px-3 sm:px-6 py-3 sm:py-4">
                      <div className="flex items-center gap-1.5 sm:gap-2">
                        {validAddress ? (
                          ...
                          <span className="text-gray-500 font-mono truncate max-w-[200px] text-xs sm:text-sm">
                            {truncatedAddress}
                          </span>
                        )}
                        <button

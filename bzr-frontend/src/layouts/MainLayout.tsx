...
const navItems = [
  ...
  { label: "CEX Trades", path: "/cex-trades" },
...
export const MainLayout: React.FC = () => {
  ...
  const footerSocialLinks = useMemo(
    () =>
      (config.footerSocialLinks || []).map((link) => ({
        ...link,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
            ...
            {footerMenus.map((section) => {
              ...
              const iconMap = {
                ...
                whitepaper: FileText,
              } as Record<SocialIconKey | "twitter", React.ElementType>;
                  ...
                  {isCommunity ? (
                    <ul className="space-y-3">
                      {footerSocialLinks.map((link) => {
                        const Icon = iconMap[(link.icon || resolveSocialIconKey(link)) as SocialIconKey | "twitter"] || Globe;
                        return (

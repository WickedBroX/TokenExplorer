        ...
        <div className="block md:hidden">
          <div className="divide-y divide-gray-100">
            {loadingHolders ? (
              ...
              <div className="p-4 text-sm text-red-500 text-center bg-white">Error loading holders: {typeof holdersError === 'string' ? holdersError : holdersError.message}</div>
            ) : sortedHolders.length === 0 ? (
              <div className="p-4 text-sm text-gray-500 text-center bg-white">No holders found matching your criteria.</div>
            ) : (
                  ...
                  <div key={holder.TokenHolderAddress} className="p-3 bg-white">
                    <div className="flex items-start justify-between gap-2 min-w-0">
                      <div className="flex items-center gap-1.5 min-w-0 flex-1">
                        <div className="text-gray-500 font-mono text-xs flex-shrink-0">#{rank}</div>
                        <a
                        ...
                        >
                          {copiedAddress === holder.TokenHolderAddress ? <Check className="w-3.5 h-3.5 text-green-500"/> : <Copy className="w-3.5 h-3.5" />}
                        </button>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <div className="font-medium text-gray-900 text-sm whitespace-nowrap">{balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                        <div className="text-xs text-gray-600 whitespace-nowrap">{formatUsdValue(valueUsd)}</div>
                      </div>
                    </div>
                  </div>
                );
          ...
          <table className="w-full text-xs sm:text-sm text-left min-w-[480px]">
            <thead className="bg-gray-50 text-gray-500 font-medium border-b border-gray-200">
              <tr>
                ...
                <th className="px-3 sm:px-6 py-2 sm:py-3 text-right">Quantity</th>
              ...
              {loadingHolders ? (
                ...
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td className="px-3 sm:px-6 py-3 sm:py-4"><div className="h-4 w-6 sm:w-8 bg-gray-200 rounded"></div></td>
                    <td className="px-3 sm:px-6 py-3 sm:py-4"><div className="h-4 w-32 sm:w-48 bg-gray-200 rounded"></div></td>
                    <td className="px-3 sm:px-6 py-3 sm:py-4"><div className="h-4 w-20 sm:w-24 bg-gray-200 rounded ml-auto"></div></td>
                    <td className="px-3 sm:px-6 py-3 sm:py-4"><div className="h-4 w-16 sm:w-20 bg-gray-200 rounded ml-auto"></div></td>
                  </tr>
                ))
              ) : holdersError ? (
                <tr>
                   <td colSpan={4} className="px-3 sm:px-6 py-8 sm:py-12 text-center text-sm text-red-500">
                      Error loading holders: {typeof holdersError === 'string' ? holdersError : holdersError.message}
                   </td>
                </tr>
              ) : sortedHolders.length === 0 ? (
                <tr>
                   <td colSpan={4} className="px-3 sm:px-6 py-8 sm:py-12 text-center text-sm text-gray-500">

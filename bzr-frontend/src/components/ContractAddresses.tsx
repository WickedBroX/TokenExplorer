...
import { Box, Search, Copy, Check, ExternalLink, ChevronDown, ChevronUp } from 'lucide-react';
...
export const ContractAddresses: React.FC<ContractAddressesProps> = ({ tokenAddress }) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [copiedChain, setCopiedChain] = useState<string | null>(null);
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(new Set(['layer1', 'layer2', 'sidechain']));

  // Contract links organized by type
  const contractLinks: ContractLink[] = useMemo(() => [
    // Layer 1 Networks
    { name: 'Ethereum', url: `https://etherscan.io/address/${tokenAddress}`, type: 'layer1', active: true },
    { name: 'BSC', url: `https://bscscan.com/address/${tokenAddress}`, type: 'layer1', active: true },
    { name: 'Avalanche', url: `https://subnets.avax.network/c-chain/address/${tokenAddress}`, type: 'layer1', active: true },
    { name: 'Cronos', url: `https://cronoscan.com/address/${tokenAddress}`, type: 'layer1', active: true },
    
    // Layer 2 Solutions
    { name: 'Polygon', url: `https://polygonscan.com/address/${tokenAddress}`, type: 'layer2', active: true },
    { name: 'Arbitrum', url: `https://arbiscan.io/address/${tokenAddress}`, type: 'layer2', active: true },
    { name: 'Optimism', url: `https://optimistic.etherscan.io/address/${tokenAddress}`, type: 'layer2', active: true },
    { name: 'Base', url: `https://basescan.org/address/${tokenAddress}`, type: 'layer2', active: true },
    { name: 'zkSync', url: `https://explorer.zksync.io/address/${tokenAddress}`, type: 'layer2', active: true },
    
    // Sidechains / Other
    { name: 'Mantle', url: `https://mantlescan.xyz/address/${tokenAddress}`, type: 'sidechain', active: true },
  ], [tokenAddress]);
  ...
  const filteredCount = Object.values(groupedChains).reduce((sum, arr) => sum + arr.length, 0);
    ...
    <div className="bg-white shadow-xl rounded-lg p-4 md:p-6 border border-gray-100">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base md:text-lg font-semibold text-gray-900">Contract Addresses</h3>
        </div>
      ...
      <div className="mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
        <span className="text-xs text-gray-500 uppercase tracking-wide block mb-1">Token Address</span>
      ...
      <div className="space-y-3">
        {(Object.keys(groupedChains) as Array<keyof typeof groupedChains>).map((groupKey) => {
          ...
          return (
            <div key={groupKey} className="border border-gray-200 rounded-lg overflow-hidden">
              ...
              >
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full bg-gradient-to-r ${groupColors[groupKey]}`}></div>
                  ...
                  {chains.map((link) => (
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full bg-gradient-to-br ${groupColors[groupKey]} flex items-center justify-center shadow-sm flex-shrink-0`}>
      ...
      {filteredCount === 0 && (
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-500">No blockchains found matching "{searchQuery}"</p>
          <button
